...
class Restorer {
	...
	private function restoreDeleted($userId) {
		...
		$restoreMethod = new \ReflectionMethod(Trashbin::class, 'restore');
		$numRestoreParams = $restoreMethod->getNumberOfParameters();

		foreach ($this->del as $item) {
			$itemToRestore = '/' . \ltrim(\pathinfo($item['path'], PATHINFO_BASENAME) . '.d' . $item['ts'], '/');

			/*
			 * A change to Trashbin::restore happened after core 10.11.
			 * Since core 10.12, Trashbin::restore only takes 2 parameters.
			 * To be backward-compatible, do the new way of calling only if the
			 * number of parameters is 2.
			 */
			if ($numRestoreParams === 2) {
				$targetLocation = $item['path'];
				$restoreResult = Trashbin::restore($itemToRestore, $targetLocation);
			} else {
				$targetLocation = \pathinfo($item['path'], PATHINFO_BASENAME);
				// @phpstan-ignore-next-line
				$restoreResult = Trashbin::restore($itemToRestore, $targetLocation, $item['ts']);
			}

			if (!$restoreResult) {
				$result['errors'][] = $itemToRestore;
			} else {
				// FIXME: update file cache
				$result['restored'][] = $itemToRestore;
			}
		}
